...
# Read in the list of differentially methylated regions between blood naive 
# Tconvs and blood naive Tregs.
...
# Read in the list of differential peaks between blod naive Tconvs and blood 
# naive Tregs.
...
atac_reg$signature_class <- sapply(atac_reg$avg_log2FC, FUN = function(x) {
  paste0(ifelse(x < 0, yes = "Blood_naive_Treg", no = "Blood_naive_Tconv"), 
...
# Read in the list of differentially expressed genes between blood naive Tconvs 
...
genes_gr <- GenomicRanges::makeGRangesFromDataFrame(genes,
...
genes$tss_loc <- sapply(1:nrow(genes), FUN = function(x) {
  ifelse(genes$strand[x] == "+", 
         yes = genes$chromStart[x],
         no = genes$chromEnd[x])
})
regions_2kb_start <- sapply(1:nrow(genes), FUN = function(x) {
  ifelse(genes$strand[x] == "+",
         yes = genes$tss_loc[x] - 2000,
         no = genes$tss_loc[x] + 1)
})
regions_2kb_end <- sapply(1:nrow(genes), FUN = function(x) {
  ifelse(genes$strand[x] == "+",
         yes = genes$tss_loc[x] - 1,
         no = genes$tss_loc[x] + 2000)
})
regions_2kb <- data.frame(chr = genes$X.chrom,
...
atac_genes_overlaps <- findOverlaps(atac_reg_overlapping_gr, genes_gr, 
                                    ignore.strand = T)
genes_query_hits <- from(atac_genes_overlaps)
genes_subject_hits <- to(atac_genes_overlaps)
atac_2kb_overlaps <- findOverlaps(atac_reg_overlapping_gr, regions_2kb_gr, 
                                  ignore.strand = T)
...
for (i in 1:length(atac_reg_overlapping_gr)) {
  ...
  gene_names_unique <- unique(c(this_reg_2kb_subject_names, 
                                this_reg_genes_subject_names))
  if (length(gene_names_unique) > 0) {
    overlap_type_str <- sapply(gene_names_unique, FUN = function(x) {
  ...
  gene_assignments <- c(gene_assignments, paste(gene_names_unique, 
                                                collapse = ", "))
...
avg_meth_celltype_level <- sapply(relevant_cell_types, FUN = function(x) {
...
avg_meth_celltype_level_diffs <- 
  ...
  avg_meth_celltype_level[, "Blood naive Tconv"]
...
# Invert effect sizes to make the values reflect a comparison with blood naive 
# Tconvs as the base line. Currently, blood naive Tregs are the base line.
atac_reg_overlapping$avg_log2FC_bl_tconv <- 
...
##########################################################################
# Collect all results regarding DMR-peak links.
##########################################################################
...
colnames(avg_meth_df) <- paste0("Avg_raw_meth_",
                                gsub(" ", "_", colnames(avg_meth_df)))
meth_diff_df <- as.data.frame(avg_meth_celltype_level_diffs)
colnames(meth_diff_df) <- "Avg_raw_meth_diff_bl_tconv"
meth_reg_overlapping_w_vals <- cbind(meth_reg_overlapping,
                                     avg_meth_df,
                                     meth_diff_df)
colnames(meth_reg_overlapping_w_vals) <- paste0(
  "Methylation__", colnames(meth_reg_overlapping_w_vals)
)
colnames(atac_reg_overlapping) <- paste0(
  "Accessibility__", colnames(atac_reg_overlapping)
)
combined_results <- cbind(meth_reg_overlapping_w_vals,
                          atac_reg_overlapping)
dmr_peak_links <- combined_results
...
# Identify the links.
dmr_peak_gene_links <- do.call(
  rbind, 
  lapply(1:nrow(dmr_peak_links), 
         FUN = function(x) {
           #-- Get all genes assigned to the corresponding DMR.
           dmr_genes <- strsplit(
             dmr_peak_links$Methylation__gene_assignments[x], 
             split = ", "
           )[[1]] 
           #-- Get all genes assigned to the corresponding peak.
           peak_genes <- strsplit(
             dmr_peak_links$Accessibility__gene_assignments[x], 
             split = ", "
           )[[1]]
           #-- Identify those genes that are assigned to both the DMR and the 
           #-- peak.
           intersect_genes <- intersect(dmr_genes, peak_genes)
           #-- Restrict to those genes that are significantly differentially 
           #-- expressed.
           ...
           if (length(intersect_genes_2) > 0) {
             temp_df <- do.call(rbind, 
                                lapply(intersect_genes_2, 
                                       FUN = function(y) {
                                         temp_df_2 <- dmr_peak_links[x, ]
                                         temp_df_2$Expression__gene_name <- y
                                         temp_df_2$Expression__log2FoldChange <- 
                                           diff_exp[y, "log2FoldChange"]
                                         return(temp_df_2)
                                       }))
           } else {
             temp_df <- dmr_peak_links[c(), ]
             temp_df$Expression__gene_name <- c()
             temp_df$Expression__log2FoldChange <- c()
           }
           return(temp_df)
         }))
...
diff_exp_tends <- sapply(dmr_peak_gene_links$Expression__gene_name, 
                         FUN = function(x) {
                           ...
                           tend <- paste0(
                             ifelse(log2FC < 0, yes = "Blood_naive_Treg", 
                                    no = "Blood_naive_Tconv"),
                             "__hyperexpression"
                           )
                           return(tend)
                         })
dmr_peak_gene_links$Expression__signature_class <- diff_exp_tends

# Invert effect sizes to make the values reflect a comparison with blood 
# naive Tconvs as the base line. Currently, blood naive Tregs are the base line.
dmr_peak_gene_links$Expression__log2FoldChange_bl_tconv <- 
  (-1) * dmr_peak_gene_links$Expression__log2FoldChange

# Save the DMR-peak links with their effect sizes.
dmr_peak_gene_links_outfile <- paste0(
  location, 
  "/differential_methylation/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_meth_acc_gex_corr.txt"
)
write.table(dmr_peak_gene_links, file = dmr_peak_gene_links_outfile, sep = "\t",
...
# Compute Pearson's correlation coefficients and test these using two-tailed
# tests.
effect_size_colnames <- c("Methylation__Avg_raw_meth_diff_bl_tconv",
                          "Accessibility__avg_log2FC_bl_tconv",
                          "Expression__log2FoldChange_bl_tconv")
corr_test_res_all <- do.call(
  c, 
  lapply(1:(length(effect_size_colnames) - 1), FUN = function(x) {
    lapply((x + 1):length(effect_size_colnames), FUN = function(y) {
      corr_test_res <- cor.test(
        dmr_peak_gene_links[, effect_size_colnames[x]],
        dmr_peak_gene_links[, effect_size_colnames[y]],
        method = "pearson",
        alternative = "two.sided")
      exact_coef <- corr_test_res$estimate
      exact_pval <- corr_test_res$p.value
      return(list(full_res = corr_test_res, coefficient = exact_coef, 
                  pval = exact_pval))
      })
    }))
names(corr_test_res_all) <- c("Meth_vs_Acc", "Meth_vs_Gex", "Acc_vs_Gex")


# Save the results from the correlation test.
corr_test_res_outfile <- paste0(
  location, 
  "/differential_methylation/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_meth_acc_gex_corr_test_res.rds"
)
saveRDS(corr_test_res_all, file = corr_test_res_outfile)

# Generate the plot.
corr_plot <- ggplot(dmr_peak_gene_links) +
  aes(x = Methylation__Avg_raw_meth_diff_bl_tconv,
      y = Accessibility__avg_log2FC_bl_tconv,
      colour = Expression__log2FoldChange_bl_tconv) +
  ...
  geom_text(
    x = 0.6 *
      max(dmr_peak_gene_links$Methylation__Avg_raw_meth_diff_bl_tconv),
    y = 0.9 * max(dmr_peak_gene_links$Accessibility__avg_log2FC_bl_tconv),
    colour = "black",
    size = 2,
    label = paste0(
      "Meth. vs. Acc.: ",
      "r = ", round(corr_test_res_all$Meth_vs_Acc$coefficient, 3), "; ",
      "P = ", signif(corr_test_res_all$Meth_vs_Acc$pval, 3), "\n",
      "Meth. vs. Gex.: ",
      "r = ", round(corr_test_res_all$Meth_vs_Gex$coefficient, 3), "; ",
      "P = ", signif(corr_test_res_all$Meth_vs_Gex$pval, 3), "\n",
      "Acc. vs. Gex.: ",
      "r = ", round(corr_test_res_all$Acc_vs_Gex$coefficient, 3), "; ",
      "P = ", signif(corr_test_res_all$Acc_vs_Gex$pval, 3), "\n")) +
  xlab(
    "Raw methylation difference\n(Blood naive Tconvs vs. Blood naive Tregs)"
  ) +
  ylab(
    "Chr. accessibility log2(FC)\n(Blood naive Tconvs vs. Blood naive Tregs)"
  ) +
  theme_classic() +
...
corr_plot_pdf <- paste0(
  plot_outdir, 
  "/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_meth_acc_gex_corr.pdf"
)
corr_plot_rds <- paste0(
  plot_rds_outdir, 
  "/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_meth_acc_gex_corr.rds"
)
pdf(corr_plot_pdf, width = 5.5, height = 4)
print(corr_plot)
dev.off()
...
######################################################################
# Generate a correlation plot with gene names as labels.
######################################################################
...
corr_plot <- ggplot(dmr_peak_gene_links) +
  ...
  geom_text(
    ...
    label = paste0(
      ...
      "r = ", round(corr_test_res_all$Acc_vs_Gex$coefficient, 3), "; ",
      "P = ", signif(corr_test_res_all$Acc_vs_Gex$pval, 3), "\n")) +
  geom_text_repel(mapping = aes(label = Expression__gene_name),
                  size = 1,
...
corr_plot_pdf <- paste0(
  plot_outdir, 
  "/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_meth_acc_gex_corr_w_labels.pdf"
)
corr_plot_rds <- paste0(
  plot_rds_outdir, 
  "/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_meth_acc_gex_corr_w_labels.rds"
)
pdf(corr_plot_pdf, width = 5.5, height = 4)
print(corr_plot)
dev.off()
...
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_many_cell_types_atac_logscale <- 
  function(bsseq_obj, regions_gr, extend = rep(5000, length(regions_gr)), genes, exons, 
           tss_pos, atac_bigwig_path_table, atac_bigwig_prefix, rna_bigwig_path_table, 
           add_track_bws = NA, add_track_names = NA, meth_highlight = NA, atac_highlight = NA, 
           rna_highlight = NA, gene_highlight = NA, add_track_highlight = NA,
    ...
    # This function plots smoothed methalytion values, scATAC-seq signals and RNA-seq signals 
    #   in several regions using custom plot titles (for more than two cell types).
    #   The scATAC-seq tracks will have a log scale on the y axis.
    # bsseq_obj: The BSseq object to retrieve methylation data from. In the pData, this object must contain
    #   the columns "Cell_type" and "col". All cell types present in the BSseq object will be
    #   plotted.
    # regions_gr: A GRanges object containing the regions to plot.
    ...
    void <- mclapply(1:length(regions_extend_gr), FUN = function(l) {
      ...
      if (all(!(is.na(atac_highlight)))) {
        atac_plot <- atac_plot +
          geom_rect(data = regions_to_highlight,
                    mapping = aes(xmin = start, xmax = end),
                    ymin = log10(min(atac_plotting_data$score)), 
                    ymax = 1.1 * max(atac_plotting_data$score), 
                    fill = "yellow", alpha = 0.5,
                    inherit.aes = F)
      }
      atac_plot <- atac_plot +
        geom_rect(ymin = log10(min(atac_plotting_data$score))) +
        facet_wrap(~Cell_type, ncol = 1) +
        theme_classic() +
        theme(axis.text.y = element_text(colour = "black"),
              axis.ticks.y = element_line(colour = "black"),
...
meth_gr <- makeGRangesFromDataFrame(dmr_peak_gene_links[, c(1:3, 7)],
                                    keep.extra.columns = T)
acc_gr <- StringToGRanges(dmr_peak_gene_links$Accessibility__peak_ID,
...
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_many_cell_types(
  bsseq_obj = meth_data,
  regions_gr = overl_gr[order(overl_gr$Methylation__automatic_annotation)],
  extend = 3000,
  genes = genes_gr,
  exons = exons,
  tss_pos = tss_df,
  ...
  plot_titles = paste(
    dmr_peak_gene_links$Methylation__region_ID[
      order(overl_gr$Methylation__automatic_annotation)], 
    dmr_peak_gene_links$Accessibility__peak_ID[
      order(overl_gr$Methylation__automatic_annotation)],
  ...
  outfile_pdf = paste0(
    location, 
    "/differential_methylation/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_trackplots_all.pdf"
  ),
  ncores = 10
)

# Generate a version of the track plots using a log-scaled y axis for the 
# scATAC-seq track
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_many_cell_types_atac_logscale(
  bsseq_obj = meth_data,
  regions_gr = overl_gr[order(overl_gr$Methylation__automatic_annotation)],
  ...
  outfile_pdf = paste0(
    location, 
    "/differential_methylation/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_trackplots_all_atac_logscale.pdf"
  ),
  ncores = 10
)
