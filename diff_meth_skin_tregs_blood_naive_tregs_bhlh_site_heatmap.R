...
#   blood naive Treg cells) around predicted binding sites for selected bHLH 
...
# contains predicted binding sites for the selected bHLH factors.
...
# Specify the relevant transcription factor motifs. These are the bHLH motifs
# that are already shown for the differential analysis between skin Tregs and
# blood naive Tregs and which were significantly enriched on the methylation
# level.
motifs_to_show <- c("c-Myc(bHLH)/LNCAP-cMyc-ChIP-Seq(Unpublished)/Homer",
...
avg_raw_meth_outfile <- paste0(
  location, 
  "/treg_hierarchies/diff_meth_skin_vs_naive_pred_rel_bhlh_sites_ext_",
  motif_extension, 
...
single_heat_maps <- lapply(motifs_to_show, FUN = function(x) {
  ...
  #-- raw methylation across the samples. This is because only a minority of
  #-- the predicted binding sites is differential and I need to apply some
  #-- ordering so that these can be seen in the heat map.
...
heatmap_outfile_pdf <- paste0(
  plot_outdir, 
  "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_pred_rel_bhlh_sites_heatmap.pdf")
heatmap_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_pred_rel_bhlh_sites_heatmap.rds")
rowanno_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_pred_rel_bhlh_sites_heatmap_rowanno_dfs.rds")
colanno_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_pred_rel_bhlh_sites_heatmap_colanno_df.rds")
