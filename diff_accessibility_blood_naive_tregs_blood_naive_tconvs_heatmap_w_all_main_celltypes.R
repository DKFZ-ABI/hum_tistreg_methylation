...
#   Treg cells, also visualising the other main cell types that are covered in 
#   our manuscript.
...
cell_types_atac_spelling <- c("skin_treg", "skin_tconv", "blood_ccr8_treg", 
                              "blood_naive_treg", "blood_naive_tconv")
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir, "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_other_published_celltypes.pdf")
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir, "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_other_published_celltypes.rds")
...
sc_data_cd4_restr <- subset(
  ...
  subset = (treg_tconv_annot == "blood_naive_treg" & donor %in% blood_donors) |
    ...
    (treg_tconv_annot == "skin_tconv" & donor %in% skin_donors)  |
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir, 
  "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_other_published_celltypes_by_donor.pdf"
)
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_other_published_celltypes_by_donor.rds"
)
pdf(signature_heatmap_outfile_pdf, width = 12, height = 10)
eval(parse(text = heatmap_drawing_string))
dev.off()
