# This script generates track plots for TE regions that are associated with
#   interesting genes.
...
# Read in the table of TEs that are associated with interesting genes.
rel_te_file <- paste0(
  location,
...
# Generate a GRanges object for the interesting TE insertion sites.
# TE insertion regions have already been converted to 1-based closed intervals
# in the previous step.
rel_te_gr <- makeGRangesFromDataFrame(
  rel_te,
  ignore.strand = T,
...
dmrs_gr <- makeGRangesFromDataFrame(dmr_table[dmr_table$automatic_annotation == "Skin_Treg__hypomethylation", ], keep.extra.columns = T)
...
# Generate a GRanges object for the differential peaks between skin Tregs
# and blood naive Tregs.
diffpeaks_gr <- StringToGRanges(rownames(diff_acc[diff_acc$significant, ]), starts.in.df.are.0based = T)
seqlevelsStyle(diffpeaks_gr) <- "NCBI"
...
########################################################################
# Generate track plots for the annotated insertion sites.
########################################################################
...
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_cpg_col <-
  function(bsseq_obj,
           regions_gr,
    ...
    #   in several regions using custom plot titles. CpGs are displayed in red if they overlap
    #   with a custom region set.
    ...
    # cpg_col_gr: GRanges object. All CpGs overlapping with a region in this object are displayed in red.
    ...
    void <- mclapply(
      ...
      FUN = function(l) {
        ...
        # If requested by the user, find those CpG sites that overlap with the
        # custom region set specified for this purpose.
        if (!(all(is.na(cpg_col_gr)))) {
          cpgs_col_overl <- findOverlaps(cpgs_in_extended_region, cpg_col_gr)
          cpgs_in_extended_region$overlaps_w_custom_set <-
            sapply(
              1:length(cpgs_in_extended_region),
              FUN = function(x) {
                x %in% from(cpgs_col_overl)
              }
            )
        }
        ...
        if (!(all(is.na(cpg_col_gr)))) {
          meth_plotting_data$Overlaps <- rep(
            cpgs_in_extended_region$overlaps_w_custom_set,
            ncol(smoothed_meth_in_regions[[l]])
          )
        }
        ...
        if (!(all(is.na(cpg_col_gr)))) {
          meth_plot <- meth_plot +
            ...
            geom_rug(
              data = meth_plotting_data[meth_plotting_data$Overlaps, ],
              y = 0,
              sides = "b",
              colour = "red",
              length = unit(0.1, "npc")
...
# Restrict the interesting TE regions to those for which track plots should
# be generated. These are chosen by me together with Markus.
entry_indices_to_show <- c(298632, 249943, 255174, 16797, 16806, 250122, 290913)
rel_te_gr_subs <- rel_te_gr[entry_indices_to_show]

# Generate the track plots.
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_cpg_col(
  bsseq_obj = meth_data_relevant,
  regions_gr = rel_te_gr_subs,
  genes = genes_gr,
  exons = exons,
  tss_pos = tss_df,
  plot_titles = rep(
    paste0(
      rel_te_gr_subs$DMR_ID,
      "; ",
      rel_te_gr_subs$TE__name,
      "; ",
      rel_te_gr_subs$TE__class
    )
  ),
  atac_bigwig_path_table = atac_seq_data_overview,
  atac_bigwig_prefix = b330_space,
  rna_bigwig_path_table = rna_sample_mapping,
  meth_highlight = dmrs_gr,
  atac_highlight = diffpeaks_gr,
  gene_highlight = rel_te_gr,
  cpg_col_gr = rel_te_gr,
  outfile_pdf = paste0(
    location,
    "/te_analysis/gen_loc_track_plots_interesting_insertion_sites_chosen_by_Markus_and_Niklas.pdf"
  ),
  ncores = 1
)
