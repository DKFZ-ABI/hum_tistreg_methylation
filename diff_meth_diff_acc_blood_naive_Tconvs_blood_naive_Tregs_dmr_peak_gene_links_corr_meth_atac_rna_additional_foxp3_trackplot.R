# This script generates an additional track plot for a region in FOXP3, 
#   depicting blood naive Tconv cells and blood naive Treg cells.
...
rna_sample_mapping_restr <- rna_sample_mapping[
  rna_sample_mapping$Cell_type %in% relevant_cell_types, ]
relevant_cell_types_names <- sapply(relevant_cell_types, FUN = function(x) {
  unique(rna_sample_mapping_restr$Cell_type[
    rna_sample_mapping_restr$Cell_type == x])
})
...
region_to_plot <- GRanges(seqnames = "X",
                          ranges = IRanges(start = 49114000,
                                           end = 49124000))
...
meth_gr <- makeGRangesFromDataFrame(meth_reg[, c(1:3, 5:6)],
                                    keep.extra.columns = T)
acc_gr <- StringToGRanges(rownames(atac_reg),
                          starts.in.df.are.0based = T)
seqlevelsStyle(acc_gr) <- "NCBI"
overl_gr <- intersect(meth_gr, acc_gr)
assert(all(width(overl_gr) >= 1))

# Generate the track plot.
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_many_cell_types(
  bsseq_obj = meth_data,
  regions_gr = region_to_plot,
  extend = 0,
  genes = genes_gr,
  exons = exons,
  tss_pos = tss_df,
  ...
  outfile_pdf = paste0(
    plot_outdir, 
    "/diff_meth_diff_acc_diff_gex_naivetconv_naivetreg_dmr_peak_gene_links_trackplots_additional_foxp3_region.pdf"
  ),
  ncores = 1
)
...
############################################################################
# Generate an expression bar chart.
############################################################################

# Identify the RNA samples corresponding to the relevant cell types.
relevant_rna_samples <- rna_sample_mapping_restr$Sample
...
for (gene in "FOXP3") {
  ...
  tpm_gene_melted$Cell_type <- factor(sapply(tpm_gene_melted$Sample, 
                                             FUN = function(x) {
                                               cell_type_id <- rna_sample_mapping$Cell_type[
                                                 rna_sample_mapping$Sample == x]
                                               return(relevant_cell_types_names[cell_type_id])
  ...
  expr_plot_pdf <- paste0(plot_outdir, 
                          "/blood_naive_treg_blood_naive_tconv_expr_",
                          gene,
                          ".pdf")
