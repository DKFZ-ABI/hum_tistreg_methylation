# This script generates track plots for the blood CCR8+ Treg cell positioning in
#   slected DMR-peak-gene links.
...
# Read in the list of selected DMR-peak-gene links.
link_list_file <- paste0(
  location, 
  "/treg_hierarchies/diff_meth_diff_acc_diff_expr_dmr_peak_gene_links_w_ccr8_positioning_selcted_list_2023-12-19.xlsx"
)
link_list <- read_xlsx(link_list_file)
...
############################################################################
# Generate track plots fo the selected DMR-peak-gene links.
############################################################################
...
# the DMR and the peak in each selected DMR-peak-gene link.
meth_gr <- makeGRangesFromDataFrame(do.call(
  rbind, lapply(1:nrow(link_list), FUN = function(x) {
    corresp_reg_id <- link_list$Methylation__region_ID[x]
    return(meth_reg[meth_reg$region_ID == corresp_reg_id, 1:3])
  })
))
acc_gr <- StringToGRanges(link_list$Accessibility__peak_ID,
                          starts.in.df.are.0based = T)
seqlevelsStyle(acc_gr) <- "NCBI"
overl_gr <- pintersect(meth_gr, acc_gr)
assert(all(width(overl_gr) >= 1))

# Generate track plots for the selected DMR-peak-gene links. In the methylation 
# track, highlight all DMRs in DMR-peak-gene links. In the ATAC track, highlight 
# all peaks in DMR-peak-gene links. In the gene track, highlight the overlap 
...
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_many_cell_types(
  ...
  plot_titles = paste(link_list$Methylation__region_ID, 
                      link_list$Accessibility__peak_ID,
                      link_list$Expression__gene_name,
                      sep = ";\n"),
  outfile_pdf = paste0(
    location, 
    "/treg_hierarchies/diff_meth_diff_acc_diff_expr_dmr_peak_gene_links_selected_trackplots_for_blood_ccr8_positioning.pdf"
  ),
  ncores = 10
)
