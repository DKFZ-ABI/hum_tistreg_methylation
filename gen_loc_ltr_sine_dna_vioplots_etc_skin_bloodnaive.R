...
#   chromatin accessibility of LTR, SINE and DNA insertion sites for skin Treg 
#   cells and blood CD45RA+ Treg cells.
...
# Specify the TE classes that are relevant for this analysis.
...
# and TE class, together with related plots.
###############################################################################

# Compute raw methylation values for all TE regions. Remove all TE occurrences
# where at least one bisulphite sample returns NA (to make the violin plots 
...
avg_raw_meth_outfile <- paste0(
  location, 
  "/te_analysis/gen_loc_ltr_sine_dna_vio_plot_skin_bloodnaive_data_meth.txt"
)
...
te_violin_plot_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_meth.pdf"
)
te_violin_plot_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_meth.rds"
)
pdf(te_violin_plot_pdf, width = 5, height = 6)
...
dot_plot_data <- do.call(rbind, lapply(cell_types, FUN = function(x) {
  ...
  this_celltype_df <- do.call(rbind, lapply(relevant_tes, FUN = function(y) {
    this_class_df <- do.call(rbind, lapply(corresp_samp, FUN = function(z) {
      meth_vals <- avg_raw_meth_df[avg_raw_meth_df$V12 == y, z]
...
dot_plot_data$TE_class <- factor(dot_plot_data$TE_class, levels = relevant_tes)
...
dot_plot_meth_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_meth_related_dotplot.pdf"
)
dot_plot_meth_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_meth_related_dotplot.rds"
)
pdf(dot_plot_meth_pdf, width = 3, height = 4)
...
########## Related diagram 2: 
# For each considered TE insertion site, the difference between mean methylation
# in each cell type is computed. The distribution of those differences is
# shown by a ridge plot.
cell_type_1_samp <- rownames(meth_data_rel_pdata)[
  meth_data_rel_pdata$Cell_type == cell_types[1]
]
cell_type_2_samp <- rownames(meth_data_rel_pdata)[
  meth_data_rel_pdata$Cell_type == cell_types[2]
]
ridge_plot_data <- do.call(rbind, lapply(relevant_tes, FUN = function(x) {
  corresp_df <- avg_raw_meth_df[avg_raw_meth_df$V12 == x, ]
  meth_diffs <- do.call(rbind, mclapply(1:nrow(corresp_df), FUN = function(y) {
    ...
    temp_df <- data.frame(TE_class = x, meth_diff = diff_to_show)
    temp_df <- cbind(temp_df, corresp_df[y, ])
    return(temp_df)
  }, mc.cores = 10))
  return(meth_diffs)
}))
ridge_plot_data$TE_class <- factor(ridge_plot_data$TE_class, 
                                   levels = relevant_tes)
te_counts <- do.call(rbind, lapply(relevant_tes, FUN = function(x) {
  num_pos <- length(which(
    ridge_plot_data$meth_diff[ridge_plot_data$TE_class == x] > 0
  ))
  num_neg <- length(which(
    ridge_plot_data$meth_diff[ridge_plot_data$TE_class == x] < 0
  ))
  temp_df <- data.frame(TE_class = factor(x, levels = relevant_tes),
                        meth_diff = c(min(ridge_plot_data$meth_diff) + 0.4,
                                      max(ridge_plot_data$meth_diff) - 0.4),
                        text = paste0(c(num_neg, num_pos),
                                      " insertion sites"))
  return(temp_df)
}))
te_ridge_plot_meth <- ggplot(ridge_plot_data) +
  ...
  geom_text(data = te_counts, mapping = aes(label = text), y = 2.5) +
  facet_wrap(~ TE_class, ncol = 1) +
  theme_classic() +
...
ridge_plot_meth_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_meth_related_ridgeplot.pdf"
)
ridge_plot_meth_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_meth_related_ridgeplot.rds"
)
pdf(ridge_plot_meth_pdf, width = 4, height = 6)
...
# type and element class. The normalised accessibility values are based on 
# peaks that overlap with the respective TEs.
################################################################################
...
peaks_by_tes <- do.call(rbind, lapply(relevant_tes, FUN = function(x) {
  te_locs <- rmsk_filt_specialtes_gr[rmsk_filt_specialtes_gr$V12 == x]
...
# accessibility (per donor and cell type). Save these values.
...
peaks_by_tes_outfile <- paste0(
  location, 
  "/te_analysis/gen_loc_ltr_sine_dna_vio_plot_skin_bloodnaive_data_atac.txt"
)
...
te_violin_plot_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_atac.pdf"
)
te_violin_plot_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_atac.rds"
)
pdf(te_violin_plot_pdf, width = 5, height = 6)
...
########## Related diagram 1: 
# Dot plot showing, for every sample, the median of accessibility values across 
# the TE-overlapping peaks.
dot_plot_data <- do.call(
  rbind, 
  lapply(cell_types_atac_naming, FUN = function(x) {
...
dot_plot_atac_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_atac_related_dotplot.pdf"
)
dot_plot_atac_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_atac_related_dotplot.rds"
)
pdf(dot_plot_atac_pdf, width = 3, height = 4)
...
########## Related diagram 2: 
# For each considered TE-overlapping peak, the difference between mean
#   accessibility in each cell type is computed. The distribution of those 
#   differences is shown by a ridge plot.
cell_type_1_samp <- unique(peaks_by_tes_melted$Celltype_donor_comb[
  peaks_by_tes_melted$Cell_type == cell_types_atac_naming[1]
])
cell_type_2_samp <- unique(peaks_by_tes_melted$Celltype_donor_comb[
  peaks_by_tes_melted$Cell_type == cell_types_atac_naming[2]
])
ridge_plot_data <- do.call(rbind, lapply(relevant_tes, FUN = function(x) {
  corresp_df <- peaks_by_tes_melted[peaks_by_tes_melted$TE == x, ]
  assert(length(unique(corresp_df$Peak)) == 
           nrow(corresp_df) / (length(c(cell_type_1_samp, cell_type_2_samp))))
  corresp_peaks <- unique(corresp_df$Peak)
  acc_diffs <- do.call(rbind, mclapply(corresp_peaks, FUN = function(y) {
...
te_counts <- do.call(rbind, lapply(relevant_tes, FUN = function(x) {
  num_pos <- length(which(
    ridge_plot_data$acc_diff[ridge_plot_data$TE_class == x] > 0
  ))
  num_neg <- length(which(
    ridge_plot_data$acc_diff[ridge_plot_data$TE_class == x] < 0
  ))
  temp_df <- data.frame(TE_class = factor(x, levels = relevant_tes),
                        acc_diff = c(min(ridge_plot_data$acc_diff) + 0.4,
                                     max(ridge_plot_data$acc_diff) - 0.4),
                        text = paste0(c(num_neg, num_pos),
                                      " peaks"))
  return(temp_df)
}))
...
ridge_plot_atac_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_atac_related_ridgeplot.pdf"
)
ridge_plot_atac_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_ltr_sine_dna_skin_bloodnaive_violinplot_atac_related_ridgeplot.rds"
)
pdf(ridge_plot_atac_pdf, width = 4, height = 6)
