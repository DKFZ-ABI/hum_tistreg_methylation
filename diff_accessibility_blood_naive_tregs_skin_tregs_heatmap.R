...
cell_types_atac_spelling <- c("skin_treg", "blood_naive_treg")
...
mean_norm_acc <- sapply(cell_types_atac_spelling, FUN = function(x) {
  ...
  norm_acc_data <- GetAssayData(sc_data_cd4_this_cell_type, assay = "scATAC_raw", slot = "data")
...
col_anno_df <- data.frame(Cell_type = factor(gsub("_", " ", cell_types_my_spelling), 
...
single_heat_maps <- lapply(custom_order, FUN = function(x) {
  row_anno_df <- data.frame(Signature = rep(x, length(which(atac_sig_ordered$Signature_category == x))))
  ...
  region_num_anno_obj <- rowAnnotation(Placeholder = anno_block(labels = paste0(region_num, "\nPeaks"), 
                                                                show_name = F,
                                                                labels_rot = 0,
...
heatmap_drawing_string <- paste0("draw(col_anno_obj %v% ",
                                 ...
                                 ", merge_legends = T)")
signature_heatmap_outfile_pdf <- paste0(plot_outdir, "/diff_acc_skin_treg_blood_naive_treg_heatmap.pdf")
signature_heatmap_outfile_rds <- paste0(plot_rds_outdir, "/diff_acc_skin_treg_blood_naive_treg_heatmap.rds")
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir, 
  "/diff_acc_skin_treg_blood_naive_treg_heatmap_by_donor.pdf"
)
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/diff_acc_skin_treg_blood_naive_treg_heatmap_by_donor.rds"
)
pdf(signature_heatmap_outfile_pdf, width = 9, height = 10)
eval(parse(text = heatmap_drawing_string))
dev.off()
