...
#   between blood naive Treg cells and blood naive Tconv cells, also showing 
#   skin Treg cells and fat Treg cells.
...
location <- "/xxx/hm_treg_bs_rgnsbg"
...
relevant_cell_types <- c("Blood naive Treg", "Blood naive Tconv", "Skin Treg", 
                         "Fat Treg")

# Read in the list of differentially methylated regions between and blood naive 
# Tregs and blood naive Tconvs.
...
meth_data <- readRDS(paste(
  location, 
  "/preprocessing_etc/quality_control_after_alignment/2022-03-14_bsseq_object_combined_all_samples_w_fattreg_smoothed.rds", 
  sep = "/"
))
meth_data <- meth_data[, unlist(lapply(relevant_cell_types, FUN = function(x) {
  which(pData(meth_data)$Cell_type == x)
}))]
...
single_heat_maps <- lapply(categories, FUN = function(x) {
  ...
  avg_raw_meth_this_signature <- getMeth(meth_data, regions = this_signature_gr, 
  ...
  row_anno_obj <- rowAnnotation(
    ...
    show_annotation_name = ifelse(x == categories[length(categories)], yes = T, 
                                  no = F),
    annotation_legend_param = list(
      title_gp = gpar(fontsize = 17, fontface = "bold"), 
  ...
  #-- Generate a text-based row annotation depicting the number of regions in 
  #-- this signature.
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir, 
  "/diff_meth_blood_naive_treg_blood_naive_tconv_heatmap_w_skin_and_fat_treg.pdf"
)
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/diff_meth_blood_naive_treg_blood_naive_tconv_heatmap_w_skin_and_fat_treg.rds"
)
signature_heatmap_rowanno_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/diff_meth_blood_naive_treg_blood_naive_tconv_heatmap_w_skin_and_fat_treg_rowanno_dfs.rds"
)
signature_heatmap_colanno_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/diff_meth_blood_naive_treg_blood_naive_tconv_heatmap_w_skin_and_fat_treg_colanno_dfs.rds"
)
