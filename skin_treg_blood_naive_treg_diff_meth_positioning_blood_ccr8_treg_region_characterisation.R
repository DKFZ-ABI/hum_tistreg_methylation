...
#   particular positioning (methylation-wise) with respect to skin Treg cells
...
# Read in the information on differentially methylated regions between
# skin Tregs and blood naive Tregs (with positioning of blood CCT8+ Tregs).
regions_file <- paste0(
  location,
...
# Blood naive Treg hypomethylation regions in which blood CCR8+ Tregs are closer
# to blood naive Tregs.
naive_hypo_ccr8_close_blood_naive <-
  regions[regions$automatic_annotation == "Blood_naive_Treg__hypomethylation" &
            regions$Cell_type_closest_to_blood_ccr8_treg == "Blood naive Treg", ]

# Blood naive Treg hypomethylation regions in which blood CCR8+ Tregs are closer
# to skin Tregs.
naive_hypo_ccr8_close_skin <-
  regions[regions$automatic_annotation == "Blood_naive_Treg__hypomethylation" &
            regions$Cell_type_closest_to_blood_ccr8_treg == "Skin Treg", ]

# Skin Treg hypomethylation regions in which blood CCR8+ Tregs are closer
# to blood naive Tregs.
skin_hypo_ccr8_close_blood_naive <-
  regions[regions$automatic_annotation == "Skin_Treg__hypomethylation" &
            regions$Cell_type_closest_to_blood_ccr8_treg == "Blood naive Treg", ]

# Skin Treg hypomethylation regions in which blood CCR8+ Tregs are closer
# to skin Tregs.
skin_hypo_ccr8_close_skin <-
  regions[regions$automatic_annotation == "Skin_Treg__hypomethylation" &
            regions$Cell_type_closest_to_blood_ccr8_treg == "Skin Treg", ]

# Skin Treg hypomethylation regions in which blood CCR8+ Tregs are substantially
# hypomethylated with respect to skin Tregs.
# These are identified as regions where the diffrence (in scaled methylation) between
# skin Tregs and blood CCR8+ Tregs is more than one standard deviation below 1.
scaled_meth_diffs_cat <- regions$Scaled_meth_diff_blood_ccr8_treg_skin_treg[
  regions$automatic_annotation == "Skin_Treg__hypomethylation"
]
scaled_meth_diffs_cat_sd <- sd(scaled_meth_diffs_cat)
skin_hypo_ccr8_hypo_wrt_skin <-
  regions[regions$automatic_annotation == "Skin_Treg__hypomethylation" &
            regions$Scaled_meth_diff_blood_ccr8_treg_skin_treg > scaled_meth_diffs_cat_sd, ]
...
file_snips <- c(
  "blood_naive_treg_hypomethylation_closer_to_blood_naive",
  "blood_naive_treg_hypomethylation_closer_to_skin",
  "skin_treg_hypomethylation_closer_to_blood_naive",
  "skin_treg_hypomethylation_closer_to_skin",
  "skin_treg_hypomethylation_even_lower_in_blood_ccr8_sd_crit"
)
...
# of the same general methylation tendency.
complement_sets <- list(
  ...
  skin_hypo_ccr8_close_blood_naive,
  regions[regions$automatic_annotation == "Skin_Treg__hypomethylation" &
            !(regions$region_ID %in% skin_hypo_ccr8_hypo_wrt_skin$region_ID), ]
)
...
##################################################################################
# Analyse enrichment of transcription factors in region sets (compared to
# the the remaining regions of the same general methylation tendency) using Homer
##################################################################################
...
for (i in 1:length(region_sets)) {
  ...
  # Decrease start positions by 1 nucleotide to make intervals 0-based and half-open.
  bed_prep_df <- complement_to_use[, 1:3]
  bed_prep_df$seqnames <- paste0("chr", bed_prep_df$seqnames)
  bed_prep_df$start <- bed_prep_df$start - 1
  bed_prep_df$Region_ID <- complement_to_use$region_ID
  bed_prep_df$Empty_col_5 <- "mock"
  bed_prep_df$Strand = "."
  # bed_prep_df <- bed_prep_df[1:20, ] # For debugging purposes.
...
###############################################################################
# Visualise some interesting results from homer's enrichment analysis against
# remaining regions.
###############################################################################

# Read in the known motifs results from homer's enrichment analysis for
# skin Treg hypomethylation regions that are closer to skin Treg against
# remaining regions.
homer_file <- paste0(
  location,
  "/treg_hierarchies/diff_meth_skin_treg_blood_naive_treg_positioning_blood_ccr8_treg_skin_treg_hypomethylation_closer_to_skin_homer_against_remaining_regions/knownResults.txt"
)
homer_res <- read.table(
  homer_file,
  sep = "\t",
  header = T,
  stringsAsFactors = F,
...
target_perc <- as.numeric(gsub("%", "", homer_res$X..of.Target.Sequences.with.Motif))
background_perc <- as.numeric(gsub("%", "", homer_res$X..of.Background.Sequences.with.Motif))
homer_res$Enrichment_score <- target_perc / background_perc

# Scale the enrichment scores so that they range from 0 to 1.
min_score <- min(homer_res$Enrichment_score)
max_score <- max(homer_res$Enrichment_score)
homer_res$Enrichment_score_scaled <- (homer_res$Enrichment_score - min_score) / (max_score - min_score)

# Save the results with computed enrichment scores.
homer_res_outfile <- paste0(
  location,
  "/treg_hierarchies/diff_meth_skin_treg_blood_naive_treg_positioning_blood_ccr8_treg_skin_treg_hypomethylation_closer_to_skin_homer_against_remaining_regions_known_res_w_enr_scores.txt"
)
write.table(
  homer_res,
  file = homer_res_outfile,
  sep = "\t",
  col.names = T,
  row.names = F,
  quote = F
...
tfs_to_show <- c(
  # Selection of bHLH factors with CACGT sub-motif
...
relevant_values <- homer_res[homer_res$Motif.Name %in% tfs_to_show, ]
...
relevant_values$Region_class <- "Skin_Treg__hypomethylation\n(blood CCD8+ Tregs are closer to skin Tregs)"
...
dot_plot_pdf <- paste0(
  plot_outdir,
  "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_positioning_blood_ccr8_treg_skin_treg_hypomethylation_closer_to_skin_homer_against_remaining_regions_dot_plot_selected_motifs.pdf"
)
dot_plot_rds <- paste0(
  plot_rds_outdir,
  "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_positioning_blood_ccr8_treg_skin_treg_hypomethylation_closer_to_skin_homer_against_remaining_regions_dot_plot_selected_motifs.rds"
)
pdf(dot_plot_pdf, width = 8, height = 8)
print(dot_plot)
dev.off()
...
for (i in 1:length(region_sets)) {
  ...
  volcano_plot_pdf <- paste0(
    plot_outdir,
    "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_positioning_blood_ccr8_treg_",
    file_snips[i],
...
# same general methylation tendency).
###########################################################################################
