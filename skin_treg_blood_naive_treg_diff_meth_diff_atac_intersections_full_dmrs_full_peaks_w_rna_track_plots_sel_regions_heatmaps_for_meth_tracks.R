...
#   plots showing DMR-peak-gene links with interesting blood CCR8+ Treg cell
#   positionings.
...
# Read in the list of DMR-peak-gene links selected by Markus.
link_list_markus_file <- paste0(
  location, 
  "/treg_hierarchies/diff_meth_diff_acc_diff_expr_dmr_peak_gene_links_w_ccr8_positioning_selcted_list_2023-12-19.xlsx"
)
link_list_markus <- read_xlsx(link_list_markus_file)
...
links_to_show_inds <- c(3, 35, 11, 52)
...
meth_gr <- makeGRangesFromDataFrame(do.call(
  rbind, lapply(links_to_show_inds, FUN = function(x) {
    corresp_reg_id <- link_list_markus$Methylation__region_ID[x]
    return(meth_reg[meth_reg$region_ID == corresp_reg_id, 1:3])
  })
))
acc_gr <- StringToGRanges(
  link_list_markus$Accessibility__peak_ID[links_to_show_inds],
...
################################################################################
# Generate heat maps showing raw. methylation for each CpG that was covered in 
# the track plots.
################################################################################

# Specify a colour palette for the cell types.
cell_type_col_palette <- c("darkorchid1", "orange", "blue")
...
single_heatmaps <- lapply(1:length(overl_gr), FUN = function(x) {
  ...
  this_region_heatmap <- Heatmap(
    ...
    column_title = ifelse(
      ...
      yes = paste(
        link_list_markus$Methylation__region_ID[links_to_show_inds[x]], 
        link_list_markus$Accessibility__peak_ID[links_to_show_inds[x]],
        link_list_markus$Expression__gene_name[links_to_show_inds[x]],
        sep = ",\n"
      ),
      no = "Custom\nFOXP3 region"
    )
...
heatmap_pdf <- paste0(
  plot_outdir, 
  "/dmr_peak_gene_links_blood_ccr8_pos_meth_heatmaps_for_trackplots.pdf"
)
heatmap_rds <- paste0(
  plot_rds_outdir, 
  "/dmr_peak_gene_links_blood_ccr8_pos_meth_heatmaps_for_trackplots.rds"
)
