...
#   between skin Treg cells and blood naive Treg cells, also showing the other
#   main cell types that appear in our manuscript.
...
relevant_cell_types <- c("Skin Treg",
                         "Skin Tconv",
                         "Blood CCR8+ Treg",
                         "Blood naive Treg",
                         "Blood naive Tconv")
...
# Generate two halves of the heat map. Reason: The wholee heat map refuses to
# render.
chunks <- list(1:9, 10:ncol(meth_data))
part_names <- c("1", "2")
for (i in 1:length(chunks)) {
  # Generate a column annotation.
  cell_types_by_sample <- pData(meth_data)$Cell_type[chunks[[i]]]
  col_anno_df <- data.frame(
    Cell_type = factor(cell_types_by_sample, levels = relevant_cell_types),
    row.names = rownames(pData(meth_data))[chunks[[i]]]
  )
  ...
  signature_heatmap_outfile_pdf <- paste0(
    plot_outdir,
    "/treg_hierarchies_diff_meth_skin_treg_blood_naive_treg_heatmap_w_other_published_celltypes_part_",
    part_names[i],
    ".pdf"
  )
