...
#   expressed between skin Treg cells and blood naive Treg cells, also
#   visualising the other main cell types that are shown in our manuscript.
...
rna_sample_mapping <- rna_sample_mapping[rna_sample_mapping$Cell_type_wo_blank %in% cell_types_rna_spelling, ]
...
rna_sig$Signature_category <- sapply(
  rna_sig$log2FoldChange,
  FUN = function(x) {
...
relevant_samples <- unlist(lapply(
  cell_types_rna_spelling,
  FUN = function(x) {
    rna_sample_mapping$Sample[rna_sample_mapping$Cell_type_wo_blank == x]
  }
))
tpm_relevant <- tpm[tpm$Gene_symbol %in% rownames(rna_sig), c("Gene_symbol", relevant_samples)]
...
rna_sig_ordered <- do.call(rbind, lapply(
  custom_order,
  FUN = function(x) {
...
set.seed(2023)
sig_classes_col_palette <- sample(rainbow(length(custom_order)), length(custom_order))
names(sig_classes_col_palette) <- custom_order


# Generate a column annotation.
cell_types_by_sample <- sapply(
  colnames(tpm_relevant),
  FUN = function(x) {
    cell_types_my_spelling[cell_types_rna_spelling == rna_sample_mapping$Cell_type_wo_blank[rna_sample_mapping$Sample == x]]
  }
)
col_anno_df <- data.frame(Cell_type = factor(
  gsub("_", " ", cell_types_by_sample),
...
single_heat_maps <- lapply(
  ...
  FUN = function(x) {
    row_anno_df <- data.frame(Signature = rep(x, length(
      which(rna_sig_ordered$Signature_category == x)
    )))
    ...
    matr_to_show <- tpm_matr_rowscaled[rownames(rna_sig_ordered)[rna_sig_ordered$Signature_category == x], ]
    this_signature_heatmap <- Heatmap(
      matr_to_show,
      col = colorRamp2(seq(
        min(tpm_matr_rowscaled),
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir,
  "/diff_expr_skin_treg_blood_naive_treg_heatmap_w_other_published_celltypes.pdf"
)
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir,
  "/diff_expr_skin_treg_blood_naive_treg_heatmap_w_other_published_celltypes.rds"
)
