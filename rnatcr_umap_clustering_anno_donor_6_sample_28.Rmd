---
title: "UMAP, clustering and cell type annotation -- Donor 6; Sample 28"
...
This R markdown performs UMAP, clustering and cell type annotation for cells from sample 28 of donor 6 (blood CD4+CD25+ T cells).
...
# Restrict the data to sample 28 (Blood CD4+CD25+ T cells).
sc_data_28 <- subset(sc_data, subset = orig.ident %in% "MD-scRNA_R28")
...
sc_data_28 <- FindVariableFeatures(sc_data_28, nfeatures = 3000)

# Generate a plot showing the variable features (With labels for the top-20.
var_feature_plot <- LabelPoints(plot = VariableFeaturePlot(sc_data_28), 
                                points = head(VariableFeatures(sc_data_28), 20), 
                                repel = T,
                                xnudge = 0, 
                                ynudge = 0,
                                force = 10)
var_feature_pdf <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_var_features_plot.pdf")
var_feature_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_var_features_plot.rds")
...
sc_data_28 <- ScaleData(sc_data_28, 
                        features = rownames(sc_data_28),
...
elbow_plot <- ElbowPlot(sc_data_28, ndims = 50)
elbow_pdf <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_elbow_plot.pdf")
elbow_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_elbow_plot.rds")
...
loadings_plot <- VizDimLoadings(sc_data_28, dims = 1:3, ncol = 3)
loadings_pdf <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_pca_loadings_plot.pdf")
loadings_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_pca_loadings_plot.rds")
...
# Save these plots.
umap_sample_pdf <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_spaces_by_sample.pdf")
umap_sample_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_spaces_by_sample.rds")
...
umap_qual_plots <- lapply(pc_numbers_to_test, FUN = function(x) {
  FeaturePlot(sc_data_28,
              reduction = paste0("UMAP_", x),
              features = c("nCount_RNA", "nFeature_RNA", "percent.mt"))
})
  
# Save these plots.
umap_qual_pdf <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_spaces_by_qual.pdf")
umap_qual_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_spaces_by_qual.rds")
...
for (pc_num in pc_nums_for_clustering) {
  sc_data_28 <- FindNeighbors(sc_data_28, 
                                    dims = 1:pc_num,
                                    force.recalc = T)
  this_pc_cluster_plots <- list()
  for (resolution in resolutions_to_test) {
    sc_data_28 <- FindClusters(sc_data_28,
                                     resolution = resolution,
                                     algorithm = 4)
    clusters_name <- paste0("Clusters_", pc_num, "_res_", resolution)
    sc_data_28[[clusters_name]] <- sc_data_28$seurat_clusters
    clusters_plot <- DimPlot(sc_data_28, 
...
for (pc_num in pc_nums_for_clustering) {
  for (resolution in resolutions_to_test) {
    ...
    outfile_snip <- paste0("/scrnatcr_donor6_sample28_umap_space_", 
                           pc_num, 
                           "_by_clusters_", 
                           pc_num, 
                           "_res_", 
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_10_by_expr_general_markers.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_10_by_expr_general_markers.rds")
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_20_by_expr_general_markers.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_20_by_expr_general_markers.rds")
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_30_by_expr_general_markers.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_30_by_expr_general_markers.rds")
png(outfile_png, width = 9, height = 15, units = "in", res = 2000)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_40_by_expr_general_markers.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_40_by_expr_general_markers.rds")
png(outfile_png, width = 9, height = 15, units = "in", res = 2000)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)

# Print the plots to the HTML oustput.
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_50_by_expr_general_markers.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_50_by_expr_general_markers.rds")
png(outfile_png, width = 9, height = 15, units = "in", res = 2000)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
sc_data_28 <- AddModuleScore(sc_data_28, 
...
for(i in 1:length(naive_ccr8_diff_list)) {
  ...
  sc_data_28[[slot_name]] <- sc_data_28[[paste0("Mod_score", i)]][, 1]
}
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_10_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_10_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.rds")
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_20_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_20_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_30_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_30_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_40_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_40_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_50_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_50_by_expr_bloodnaive_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_10_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_10_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_20_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_20_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_30_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_30_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_40_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_40_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
outfile_png <- paste0(plot_outdir, "/scrnatcr_donor6_sample28_umap_space_50_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.png")
outfile_rds <- paste0(plot_rds_outdir, "/scrnatcr_donor6_sample28_umap_space_50_by_expr_bloodmem_boodccr8_diffgenes_mod_scores.rds")
png(outfile_png, width = 9, height = 3, units = "in", res = 750)
grid.draw(arrangeGrob(grobs = plot_list, ncol = 3))
invisible(dev.off())
saveRDS(plot_list, file = outfile_rds)
...
final_annotations <- categories[c(3, 3, 1, 3, 3, 1, 3, 2, 3, 3, 3)]

# Append these annotations to the Seurat object.
cluster_assignments <- sc_data_28$Clusters_10_res_0.7
annotations_by_cell <- final_annotations[cluster_assignments]
names(annotations_by_cell) <- colnames(sc_data_28)
sc_data_28$Cell_type_annotation_10_res_0.7 <- annotations_by_cell
...
sc_data_28$Cell_type_annotation_10_res_0.7_colours <- colours_by_cell

# Generate and save a plot showing cells in UMAP space, coloured by their cell type assignments.
plotting_data <- as.data.frame(Embeddings(sc_data_28, 
                                          reduction = "UMAP_10"))
colnames(plotting_data) <- c("UMAP_1", "UMAP_2")
plotting_data$Cell_type <- factor(sc_data_28$Cell_type_annotation_10_res_0.7,
                                  levels = categories)
...
cell_type_plot_pdf <- paste0(
  plot_outdir, 
  "/scrnatcr_donor6_sample28_umap_space_10_by_final_anno_10_res_0.7.pdf"
)
...
seur_outfile <- paste0(location, 
                       "scrnatcr_donor6_sample28_seur_obj_umap_anno.rds")
saveRDS(sc_data_28, file = seur_outfile)
```