...
cell_types_rna_spelling <- c("Skin_Treg", "Fat_Treg", "Blood_naive_Treg")
...
tpm_file <- paste0(location, "/RNASeq/tpm/tpm_all_w_fattreg.txt")
...
single_heat_maps <- lapply(
  ...
  FUN = function(x) {
    ...
    matr_to_show <- tpm_matr_rowscaled[
      rownames(rna_sig_ordered)[rna_sig_ordered$Signature_category == x], 
    ]
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir,
  "/diff_expr_skin_treg_blood_naive_treg_heatmap_w_fat_treg.pdf"
)
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir,
  "/diff_expr_skin_treg_blood_naive_treg_heatmap_w_fat_treg.rds"
)
