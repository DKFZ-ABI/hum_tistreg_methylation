# This script analyses enrichments of TE genomic loci (RepeatMasker) in DMRs
...
#############################################################################
# Generate randomly shuffled versions of the DMRs.
#############################################################################

shuffle_genomic_regions_hs37d5 <- function(regions_gr, chrs_to_use, 
                                           temp_file_location, 
                                           seed_to_use,
                                           shuffled_regions_outfile,
                                           keep_shuffled_bed_on_disk = F) {
  # This function randomly shuffles genomic regions on the hs37d5 genome using 
  ...
  #   BSgenome.Hsapiens.1000genomes.hs37d5 must be installed but do not need to 
  #   be loaded. bedtools/2.29.2 must be installed.
...
region_classes <- unique(dmrs_gr$automatic_annotation)
shuffled_intervals <- lapply(region_classes, FUN = function(x){
  print(x)
  
  # Extract corresponding genomic intervals.
  intervals_gr <- dmrs_gr[dmrs_gr$automatic_annotation == x]
  ...
  shufflings_all <- mclapply(seeds, FUN = function(y) {
    shuffle_genomic_regions_hs37d5(
      regions_gr = intervals_gr, 
      chrs_to_use = c(as.character(1:22), "X"), 
      temp_file_location = paste0(location, "/te_analysis/tempdir_", y), 
      seed_to_use = y, 
      shuffled_regions_outfile = paste0(
        location, "/te_analysis/bed_", y, ".bed")
    )
  }, mc.cores = 10)
...
enrichment_results_complete <- lapply(region_classes, FUN = function(i) {
  ...
  # Identify corresponding DMRs.
  corresp_dmrs_gr <- dmrs_gr[dmrs_gr$automatic_annotation == i]
  # corresp_dmrs_gr <- corresp_dmrs_gr[1:100] # For debugging purposes.
  
  # Extract corresponding DMR shufflings.
  ...
  enrichment_dat_all_granularities <- lapply(grouping_cols, FUN = function(j) {
    ...
    compute_overl_enrichment_wrt_permutations(
      query_regions_gr = corresp_dmrs_gr, 
...
outfile_pdf <- paste0(plot_outdir, 
                      "/te_analysis_gen_loc_enrichm_dmrs_skin_naive.pdf")
