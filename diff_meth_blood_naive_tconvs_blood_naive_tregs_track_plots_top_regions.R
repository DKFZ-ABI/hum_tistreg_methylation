...
#   blood naive Tconv cells and blood naive Treg cells.
...
cell_types <- c("Blood naive Treg", "Blood naive Tconv")
...
treg_hypo_gr <- makeGRangesFromDataFrame(diff_reg[diff_reg$automatic_annotation == "Blood_naive_Treg__hypomethylation", ], keep.extra.columns = T)
tconv_hypo_gr <- makeGRangesFromDataFrame(diff_reg[diff_reg$automatic_annotation == "Blood_naive_Tconv__hypomethylation", ], keep.extra.columns = T)

# Compute sample-level average raw methylation values for these regions.
treg_hypo_avg_raw_meth <- getMeth(
  meth_data_relevant,
  regions = treg_hypo_gr,
  type = "raw",
  what = "perRegion"
)
tconv_hypo_avg_raw_meth <- getMeth(
  meth_data_relevant,
  regions = tconv_hypo_gr,
  type = "raw",
  what = "perRegion"
)

# Aggregate these into cell-level values.
treg_hypo_avg_raw_meth_aggr <- sapply(
  cell_types,
  FUN = function(x) {
    celltype_subs <- treg_hypo_avg_raw_meth[, grep(x, colnames(treg_hypo_avg_raw_meth), fixed = T)]
    return(rowMeans(celltype_subs, na.rm = T))
  }
)
tconv_hypo_avg_raw_meth_aggr <- sapply(
  cell_types,
  FUN = function(x) {
    celltype_subs <- tconv_hypo_avg_raw_meth[, grep(x, colnames(tconv_hypo_avg_raw_meth), fixed = T)]
    return(rowMeans(celltype_subs, na.rm = T))
  }
)

# Compute differences in aggregated raw mthylation values betwen the two cell types.
treg_hypo_diffs <- abs(rowDiffs(treg_hypo_avg_raw_meth_aggr))
tconv_hypo_diffs <- abs(rowDiffs(tconv_hypo_avg_raw_meth_aggr))
...
treg_hypo_seqnames <- as.character(seqnames(treg_hypo_gr))
treg_hypo_reg_to_keep <- which(sapply(
  1:length(treg_hypo_gr),
  FUN = function(x) {
    treg_hypo_diffs[x, 1] >= ifelse(treg_hypo_seqnames[x] != "X", yes = 0.5, no = 0.25)
  }
))
treg_hypo_top_regions_gr <- treg_hypo_gr[treg_hypo_reg_to_keep]
tconv_hypo_seqnames <- as.character(seqnames(tconv_hypo_gr))
tconv_hypo_reg_to_keep <- which(sapply(
  1:length(tconv_hypo_gr),
  FUN = function(x) {
    tconv_hypo_diffs[x, 1] >= ifelse(tconv_hypo_seqnames[x] != "X", yes = 0.5, no = 0.25)
  }
))
tconv_hypo_top_regions_gr <- tconv_hypo_gr[tconv_hypo_reg_to_keep]

# Collect and save all data.
treg_hypo_top_regions_df <- cbind(
  as.data.frame(treg_hypo_top_regions_gr),
  data.frame(Abs_raw_meth_diff = treg_hypo_diffs[treg_hypo_reg_to_keep, 1])
)
treg_hypo_top_regions_df_outfile <- paste0(
  location,
  "/differential_methylation/diff_meth_blood_naive_tconv_blood_naive_treg_blood_naive_treg_hypomethylation_top_regions_cutoff_0.5_0.25.txt"
)
write.table(
  treg_hypo_top_regions_df,
  file = treg_hypo_top_regions_df_outfile,
  sep = "\t",
  row.names = F,
  col.names = T,
  quote = F
)
tconv_hypo_top_regions_df <- cbind(
  as.data.frame(tconv_hypo_top_regions_gr),
  data.frame(Abs_raw_meth_diff = tconv_hypo_diffs[tconv_hypo_reg_to_keep, 1])
)
tconv_hypo_top_regions_df_outfile <- paste0(
  location,
  "/differential_methylation/diff_meth_blood_naive_tconv_blood_naive_treg_blood_naive_tconv_hypomethylation_top_regions_cutoff_0.5_0.25.txt"
)
write.table(
  tconv_hypo_top_regions_df,
  file = tconv_hypo_top_regions_df_outfile,
  sep = "\t",
  row.names = F,
  col.names = T,
  quote = F
...
plot_meth_and_other_signal_many_regions_custom_titles_atac_log <- function(bsseq_obj,
                                                                           regions_gr,
                                                                           extend = rep(5000, length(regions_gr)),
                                                                           genes,
                                                                           exons,
  ...
  void <- mclapply(
    ...
    FUN = function(l) {
      ...
      atac_plot <- ggplot(atac_plotting_data) +
        ...
        scale_y_log10(expand = expansion(mult = c(0, 0.05)), name = "Binned scATAC-seq\nread count [RPKM]") +
        scale_fill_manual(
          breaks = levels(atac_plotting_data$Cell_type),
          values = c(colour_1, colour_2, "black"),
          name = "Cell type"
...
# Generate methylation track plots (once with normal scATAC-seq y axis and once
# with log-scaled scATAC-seq y-xis.
region_set_list <- list(treg_hypo_top_regions_gr, tconv_hypo_top_regions_gr)
file_snips <- c("blood_naive_treg_hypomethylation",
                "blood_naive_tconv_hypomethylation")
for (i in 1:2) {
  plot_meth_and_other_signal_many_regions_custom_titles(
    ...
    outfile_pdf = paste0(
      location,
      "/differential_methylation/diff_meth_blood_naive_tconv_blood_naive_treg_",
      file_snips[i],
      "_top_regions_cutoff_0.5_0.25_track_plots.pdf"
    ),
    ncores = 10
  )
  plot_meth_and_other_signal_many_regions_custom_titles_atac_log(
    bsseq_obj = meth_data_relevant,
    regions_gr = region_set_list[[i]],
    genes = genes_gr,
    exons = exons,
    ...
    outfile_pdf = paste0(
      ...
      "/differential_methylation/diff_meth_blood_naive_tconv_blood_naive_treg_",
      file_snips[i],
      "_top_regions_cutoff_0.5_0.25_track_plots_atac_logscale.pdf"
    ),
    ncores = 10
  )
}
