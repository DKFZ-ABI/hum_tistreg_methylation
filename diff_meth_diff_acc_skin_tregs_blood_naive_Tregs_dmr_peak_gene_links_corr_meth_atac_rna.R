# This R script looks at DMR-peak-gene links in the comparison between skin 
#   Treg cells and blood naive Treg cells and analyses correlations between 
#   differential methylation, differential accessibility and differential 
#   expression.
...
relevant_cell_types_atac_naming <- c("blood_naive_treg", "skin_treg")
...
dmr_peak_gene_links$Expression__log2FoldChange_bl_blood_naive <- 
...
# tests.
effect_size_colnames <- c("Methylation__Avg_raw_meth_diff_bl_blood_naive",
                          "Accessibility__avg_log2FC_bl_blood_naive",
                          "Expression__log2FoldChange_bl_blood_naive")
corr_test_res_all <- do.call(c, lapply(1:(length(effect_size_colnames) - 1), 
                                       FUN = function(x) {
...
corr_test_res_outfile <- paste0(
  location, 
  "/treg_hierarchies/diff_meth_diff_acc_overlapping_dmr_peak_gene_links_meth_acc_gex_corr_test_res.rds"
)
saveRDS(corr_test_res_all, file = corr_test_res_outfile)

# Generate the plot.
corr_plot <- ggplot(dmr_peak_gene_links) +
  aes(x = Methylation__Avg_raw_meth_diff_bl_blood_naive,
      y = Accessibility__avg_log2FC_bl_blood_naive,
      colour = Expression__log2FoldChange_bl_blood_naive) +
  ...
  geom_text(
    x = 0.6 *
      max(dmr_peak_gene_links$Methylation__Avg_raw_meth_diff_bl_blood_naive),
    y = 0.9 * max(dmr_peak_gene_links$Accessibility__avg_log2FC_bl_blood_naive),
    colour = "black",
    size = 2,
    label = paste0(
      "Meth. vs. Acc.: ",
...
corr_plot_pdf <- paste0(
  plot_outdir, 
  "/treg_hierarchies_diff_meth_diff_acc_diff_gex_dmr_peak_gene_links_meth_acc_gex_corr.pdf"
)
corr_plot_rds <- paste0(
  plot_rds_outdir, 
  "/treg_hierarchies_diff_meth_diff_acc_diff_gex_dmr_peak_gene_links_meth_acc_gex_corr.rds"
)
...
corr_plot_pdf <- paste0(
  plot_outdir, 
  "/treg_hierarchies_diff_meth_diff_acc_diff_gex_dmr_peak_gene_links_meth_acc_gex_corr_w_labels.pdf"
)
corr_plot_rds <- paste0(
  plot_rds_outdir, 
  "/treg_hierarchies_diff_meth_diff_acc_diff_gex_dmr_peak_gene_links_meth_acc_gex_corr_w_labels.rds"
)
pdf(corr_plot_pdf, width = 5.5, height = 4)
print(corr_plot)
dev.off()
...
meth_gr <- makeGRangesFromDataFrame(dmr_peak_gene_links[, 1:3])
...
plot_meth_and_other_signal_many_regions_custom_titles_custom_highlight_many_cell_types(
  ...
  plot_titles = paste(dmr_peak_gene_links$Methylation__region_ID, 
                      dmr_peak_gene_links$Accessibility__peak_ID,
                      dmr_peak_gene_links$Expression__gene_name,
                      sep = ";\n"),
  outfile_pdf = paste0(
    location, 
    "/treg_hierarchies/diff_meth_diff_acc_diff_expr_dmr_peak_gene_links_trackplots_for_skin_Tregs_blood_naive_Tregs.pdf"
  ),
  ncores = 10
)
