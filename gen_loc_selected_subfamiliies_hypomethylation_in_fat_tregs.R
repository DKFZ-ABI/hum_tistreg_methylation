# This script analyses demethylation of selected TE subfamilies in fat Treg 
#   cells.
...
# Specify the three TE subfamilies that are relevant for this analysis. These 
# subfamilies are among the ones highlighted in Figure 6a.
relevant_tes <- c("LTR3B_", "LTR18B", "HUERS-P2-int", "LTR32", "HERVL-int", 
                  "LTR1D", "MER125", "LTR14B", "HERVIP10F-int", "MER117", 
                  "LTR45B", "HERV1_LTRc")
...
avg_raw_meth_outfile <- paste0(
  location, 
  "/te_analysis/gen_loc_analysis_selected_subfamilies_demethylation_in_fat_tregs_methylation_values.txt"
)
...
te_box_plot_pdf <- paste0(
  plot_outdir, 
  "/te_analysis_gen_loc_selected_subfamilies_demethylation_in_fat_tregs.pdf"
)
te_box_plot_rds <- paste0(
  plot_rds_outdir, 
  "/te_analysis_gen_loc_selected_subfamilies_demethylation_in_fat_tregs.rds"
)
pdf(te_box_plot_pdf, width = 15, height = 10)
...
# Two-tailed Student's t test on the means of the considered TE insertion sites.
################################################################################

# Define the output file that will later contain a summary of the test results.
test_res_summary_file <- paste0(
  location, 
  "/te_analysis/gen_loc_analysis_selected_subfamilies_demethylation_in_fat_tregs_stat_test_summary.txt"
)
...
separate_vals <- lapply(relevant_tes, FUN = function(x) {
  restr_table <- avg_raw_meth_df_melted[avg_raw_meth_df_melted$V11 == x, ]
...
# Aggregate values into sample-level values by ccomputing the mean aross the
...
# two-tailed Student's t tests on the means between the considered TE insertion 
...
test_outfile <- paste0(
  location, 
  "/te_analysis/gen_loc_analysis_selected_subfamilies_demethylation_in_fat_tregs_stat_tests.rds"
)
