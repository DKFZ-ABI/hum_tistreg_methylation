# This R script identifies differentially methylated regions between skin
#   Treg cells and blood naive Treg cells by adopting my approach from the
...
output_location <- paste0(location, "/treg_hierarchies")
...
# 26,499,047 out of 27,999,538 CpG sites (94.6%) fulfil this criterion.
...
cell_type_signature_regions <- two_class_signature_extraction(
  ...
  cpg_stat_outfile_snip = paste0(output_location, "/diff_meth_skin_treg_blood_naive_treg"),
  max_cpg_dist = 300,
  min_cpg_num = 3,
  min_mean_cov = 2,
  log_reg_max_num_misclassified = 0,
  log_reg_outfile_snip = paste0(output_location, "/diff_meth_skin_treg_blood_naive_treg"),
  regions_outfile_snip = paste0(output_location, "/diff_meth_skin_treg_blood_naive_treg"),
  cats_for_dependencies_plots = interesting_categories,
  dependencies_pdf_snip = paste0(plot_outdir, "/diff_meth_skin_treg_blood_naive_treg"),
  dependencies_rds_snip = paste0(plot_rds_outdir, "/diff_meth_skin_treg_blood_naive_treg")
)
...
for (cat in interesting_categories) {
  ...
  plot_meth_and_other_signal_many_regions_custom_titles(
    ...
    outfile_pdf = paste0(
      output_location,
      "/diff_meth_skin_treg_blood_naive_treg_",
      sig_name,
      "_",
      cat,
      ext_file_snip,
