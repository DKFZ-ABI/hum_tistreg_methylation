...
#   differentially accessible between blood naive Tconv cells and blood naive 
#   Treg cells, also visualising skin Treg cells and fat Treg cells.
...
# Read in the table with peaks that are differentially accessible between blood 
# naive Tconvs and blood naive Tregs.
...
atac_sig$Signature_category <- sapply(atac_sig$avg_log2FC, FUN = function(x) {
  ifelse(x < 0, yes = "Blood_naive_Treg__hyperaccessibility", 
         no = "Blood_naive_Tconv__hyperaccessibility")
})

# Bring the signature peaks into a custom order.
custom_order <- c("Blood_naive_Treg__hyperaccessibility",
                  "Blood_naive_Tconv__hyperaccessibility")
atac_sig_ordered <- do.call(rbind, lapply(custom_order, FUN = function(x) {
...
cell_types_my_spelling <- c("Blood_naive_Treg", "Blood_naive_Tconv", 
                            "Skin_Treg", "Fat_Treg")
cell_types_atac_spelling <- c("blood_naive_treg", "blood_naive_tconv",
                              "skin_treg", "fat_treg")
names(cell_types_my_spelling) <- cell_types_atac_spelling

# Specify the colour code for the cell types.
cell_type_col_palette <- c("darkorchid1", "red", "blue", "green")
names(cell_type_col_palette) <- gsub("_", " ", cell_types_my_spelling)

# Specify the identity of donors that provided blood samples.
blood_donors <- c("1", "2")
fat_donors <- c("3", "4", "5")
skin_donors <- c("4", "5")
...
###############################################################################
# Generate a heat map showing accessibility scores for both differential 
# tendencies (blood naive Tconv hyperaccessibility and blood naive Treg 
# hyperaccessibility.
###############################################################################
...
sc_data_cd4_restr <- subset(
  sc_data_cd4, 
  subset = treg_tconv_annot %in% c(cell_types_atac_spelling)
)
...
mean_norm_acc <- sapply(cell_types_atac_spelling, FUN = function(x) {
  ...
  unlist(mclapply(rownames(atac_sig_ordered), FUN = function(y) {
...
col_anno_df <- data.frame(
  Cell_type = factor(gsub("_", " ", cell_types_my_spelling), 
...
single_heat_maps <- lapply(custom_order, FUN = function(x) {
  row_anno_df <- data.frame(
    Signature = rep(x, length(which(atac_sig_ordered$Signature_category == x))))
  ...
  matr_to_show <- mean_norm_acc[rownames(atac_sig_ordered)[
    atac_sig_ordered$Signature_category == x], ]
  this_signature_heatmap <- Heatmap(matr_to_show,
                                    col = colorRamp2(seq(min(mean_norm_acc), 
                                                         max(mean_norm_acc), 
                                                         length.out = 200), 
                                                     inferno(200)),
                                    cluster_rows = F,
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir, "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_skin_and_fat_treg.pdf")
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir, "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_skin_and_fat_treg.rds")
...
sc_data_cd4_restr <- subset(
  ...
  subset = (treg_tconv_annot == "blood_naive_treg" & donor %in% blood_donors) |
    (treg_tconv_annot == "blood_naive_tconv" & donor %in% blood_donors) |
...
mean_norm_acc <- do.call(
  cbind,
  lapply(cell_types_atac_spelling, FUN = function(x) {
    sc_data_cd4_this_cell_type <- subset(sc_data_cd4_restr, 
                                         subset = treg_tconv_annot == x)
    ...
    norm_acc_data_this_celltype <- sapply(
      ...
      FUN = function(y) {
        sc_data_this_donor <- subset(sc_data_cd4_this_cell_type,
                                     subset = donor == y)
        norm_acc_data <- GetAssayData(sc_data_this_donor, 
                                      assay = "scATAC_raw", slot = "data")
...
# Generate a column annotation.
col_anno_df <- data.frame(
  Cell_type = factor(sapply(colnames(mean_norm_acc), FUN = function(x) {
...
signature_heatmap_outfile_pdf <- paste0(
  plot_outdir, 
  "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_skin_and_fat_treg_by_donor.pdf"
)
signature_heatmap_outfile_rds <- paste0(
  plot_rds_outdir, 
  "/diff_acc_blood_naive_tconv_blood_naive_treg_heatmap_w_skin_and_fat_treg_by_donor.rds"
)
pdf(signature_heatmap_outfile_pdf, width = 9, height = 10)
eval(parse(text = heatmap_drawing_string))
dev.off()
